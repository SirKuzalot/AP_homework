...
class LogisticRegression:
    ...
    def update_weights(self, X, y):
        ...
        for x, label in zip(X, y):
            ...
            self.W -= self.eta * np.outer(gradient, x)  # Weight update
...
def train_logistic_regression(X_train, y_train, X_val, y_val, X_test, y_test, epochs=100, eta=0.001):
    num_classes = 6
    input_dim = X_train.shape[1]
    model = LogisticRegression(num_classes, input_dim, eta)
...
if __name__ == "__main__":
    ...
    if args.model == 'perceptron':
        ...
        train_accuracies, val_accuracies = train_logistic_regression(
            X_train, y_train, X_val, y_val, X_test, y_test, args.epochs
        )
        plot_accuracies(train_accuracies, val_accuracies)
